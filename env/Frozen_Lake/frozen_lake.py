...
class FrozenLakeEnv(Env):
    ...
    * `reward_schedule=(1, 0, 0)`: Used to specify reward amounts for reaching certain tiles.
    ...
    def __init__(
        ...
        reward_schedule: tuple[int, int, int] = (0, 0, 0),
    ):
        ...
        self.P = {(row, col): {a: [] for a in range(nA)} for row in range(nrow) for col in range(ncol)}
        #print(self.P)
        ...
        self.observation_space = spaces.Box(low=0, high=nS - 1, shape=(2,), dtype=np.int32)
    ...
    def step(self, a):
        ...
        if isinstance(_s, np.ndarray):
            _s = int(_s[0]) if _s.size == 1 else tuple(map(int, _s))

        tup = (_s // self.ncol, _s % self.ncol)
        # print(tuple, type(tuple))
        #print(self.P[tup])
        #print(a)
        transitions = self.P[tup][a]
        ...
        return np.array([row, col], dtype=np.int32), r, t, False, {"prob": p}
    ...
    def reset(
        ...
        *,
        ...
        return np.array([row, col], dtype=np.int32), {"prob": 1}
    ...
    def _render_gui(self, mode):
        ...
        if self.camp_img is None:
            try:
                file_name = path.join(path.dirname(__file__), "img/camp.png")
                self.camp_img = pygame.transform.scale(
                    pygame.image.load(file_name), self.cell_size
                )
            except Exception:
                # Fallback if you don't have camp.png yet
                self.camp_img = self.goal_img

        if self.camp_font is None:
            pygame.font.init()
            px = max(14, int(min(self.cell_size) * 0.5))
            self.camp_font = pygame.font.Font(None, px)
        ...
        for y in range(self.nrow):
            for x in range(self.ncol):
                ...
                tile = desc[y][x]
                if tile in (b"A", b"B", b"C"):
                    # Draw camp icon
                    self.window_surface.blit(self.camp_img, pos)

                    # Draw letter label centered on the tile
                    label = tile.decode("utf-8")
                    text = self.camp_font.render(label, True, (0, 0, 0))
                    tx = pos[0] + (self.cell_size[0] - text.get_width()) // 2
                    ty = pos[1] + (self.cell_size[1] - text.get_height()) // 2

                    # Simple white outline for readability
                    for dx, dy in [(-1,0), (1,0), (0,-1), (0,1)]:
                        outline = self.camp_font.render(label, True, (255, 255, 255))
                        self.window_surface.blit(outline, (tx + dx, ty + dy))
                    self.window_surface.blit(text, (tx, ty))
...
# obs, info = env.reset()
# done = False
# total_timesteps = 100000  # Total number of timesteps to run
